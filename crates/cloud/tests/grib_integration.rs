...
fn test_grib_gfs_metadata() {
    ...
    for msg in meta.messages.iter().take(10) {
        println!(
            "  [{}] {}.{}: {}",
            msg.index, msg.message_index.0, msg.message_index.1, msg.description
        );
    }
...
fn test_grib_gfs_read_message() {
    ...
    println!(
        "GeoTransform: origin=({:.4}, {:.4}), pixel=({:.6}, {:.6})",
...
fn test_grib_gfs_read_temperature() {
    ...
    let bbox = BBox {
        min_x: 289.0,
        min_y: -33.5, // 289 = 360-71
        max_x: 290.0,
        max_y: -33.0,
    };
    let cropped = reader
        .read_bbox_by_parameter("temperature", &bbox)
