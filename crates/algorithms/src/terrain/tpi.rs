...
/// Per-cell TPI kernel shared by the batch (`tpi`) and streaming
/// (`TpiStreaming`) paths.
///
...
/// Returns NaN when no valid neighbor exists.
///
/// The caller must guarantee that the full window lies inside `data`.
#[inline]
fn tpi_kernel(data: &Array2<f64>, row: usize, col: usize, r: isize, nodata: Option<f64>) -> f64 {
...
pub fn tpi(dem: &Raster<f64>, params: TpiParams) -> Result<Raster<f64>> {
    ...
    let output_data = par_map_rows(rows, cols, |row, out_row| {
        for (col, row_data_col) in out_row.iter_mut().enumerate() {
            ...
            *row_data_col = tpi_kernel(data, row, col, r, nodata);
        }
    });
...
/// Streaming TPI calculator implementing `WindowAlgorithm`.
///
...
impl Default for TpiStreaming {
    fn default() -> Self {
        Self { radius: 1 }
    }
}

impl surtgis_core::WindowAlgorithm for TpiStreaming {
        ...
        output
            ...
            .for_each(|(r, out_row)| {
                ...
                for (c, out_v) in out_row.iter_mut().enumerate() {
                    ...
                    *out_v = tpi_kernel(input, ir, c, r_i, nodata);
                }
            });
    }
}
