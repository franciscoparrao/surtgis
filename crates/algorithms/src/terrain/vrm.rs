...
/// Compute the unit surface normal vector at a single cell using Horn's
/// method (unnormalized gradients). Returns `(nx, ny, nz)` or `None` if the
/// cell is on the border or any 3×3 stencil neighbor is NaN.
///
/// Shared per-cell kernel for the batch (`vrm`) and streaming
/// (`VrmStreaming`) paths.
#[inline]
fn normal_at(data: &Array2<f64>, row: usize, col: usize) -> Option<(f64, f64, f64)> {
...
pub fn vrm(dem: &Raster<f64>, params: VrmParams) -> Result<Raster<f64>> {
    ...
    for row in 1..rows - 1 {
        for col in 1..cols - 1 {
            if let Some((vx, vy, vz)) = normal_at(data, row, col) {
                nx[[row, col]] = vx;
                ny[[row, col]] = vy;
                nz[[row, col]] = vz;
            }
        ...
        output
            ...
            .for_each(|(row_out, out_row)| {
                ...
                for (c, out_v) in out_row.iter_mut().enumerate() {
                    ...
                    for nr in (ir - r)..=(ir + r) {
                        for nc in (c - r)..=(c + r) {
                            if let Some((nx, ny, nz)) = normal_at(input, nr, nc) {
                                sum_x += nx;
                                sum_y += ny;
                                sum_z += nz;
                                count += 1;
                    ...
                    if count > 0 {
                        let resultant = (sum_x * sum_x + sum_y * sum_y + sum_z * sum_z).sqrt();
                        *out_v = 1.0 - resultant / count as f64;
                    } else {
                        *out_v = f64::NAN;
                    }
                }
